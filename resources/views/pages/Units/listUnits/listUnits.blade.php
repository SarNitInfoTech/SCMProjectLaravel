...
<div class="card shadow-sm border mb-6 bg-white">
    <div class="card-header p-4 border-b">
        <form method="GET" action="{{ route('units.index') }}" class="flex flex-wrap items-center justify-between gap-4">
            <h3 class="text-lg font-semibold text-gray-800">Measurement Units</h3>
            <div class="flex items-center gap-2">
                <input
                    type="text"
                    name="search"
                ...
                @if(request()->filled('search'))
                    <a href="{{ route('units.index') }}" class="px-3 py-1.5 text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 rounded">
                        Reset
                    </a>
                @endif
            </div>
        ...
        <table class="table whitespace-nowrap min-w-full">
            <thead>
                <tr class="border-b border-defaultborder">
                    <th scope="col" class="text-start">Unit Name</th>
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="text-center py-4 text-gray-500">No units found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if($units->hasPages())
        <div class="p-4 border-t">
            {{ $units->appends(request()->query())->links('pagination::tailwind') }}
        </div>
    @endif
</div>
@endsection