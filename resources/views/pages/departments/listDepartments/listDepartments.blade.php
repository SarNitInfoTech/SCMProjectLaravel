...
<div class="md:flex block items-center justify-between my-6 page-header-breadcrumb">
    <div>
        ...
        <p class="-mt-[0.2rem] mb-0 text-textmuted">Manage company organization departments</p>
    </div>
    <div class="flex items-center gap-2">
        <a href="{{ route('bulk-upload.index', ['module' => 'departments']) }}" class="px-4 py-2 text-sm bg-green-600 hover:bg-green-700 text-white rounded shadow transition-all flex items-center gap-1.5">
            <i class="bi bi-file-earmark-arrow-up"></i> Bulk Import
        </a>
        <a href="{{ route('departments.create') }}" class="px-4 py-2 text-sm bg-indigo-600 hover:bg-indigo-700 text-white rounded shadow transition-all">
...
<div class="card shadow-sm border mb-6 bg-white">
    <div class="card-header p-4 border-b">
        <form method="GET" action="{{ route('departments.index') }}" class="flex flex-wrap items-center justify-between gap-4">
            <h3 class="text-lg font-semibold text-gray-800">Departments Register</h3>
            <div class="flex items-center gap-2">
                <input
                    type="text"
                    name="search"
                ...
                @if(request()->filled('search'))
                    <a href="{{ route('departments.index') }}" class="px-3 py-1.5 text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 rounded">
                        Reset
                    </a>
                @endif
            </div>
            ...
            <tbody>
                @forelse ($departments as $dept)
                    <tr class="border-b border-defaultborder hover:bg-gray-50 transition-colors">
                        ...
                        <td>{{ $dept->updated_at ? $dept->updated_at->format('d-m-Y') : '—' }}</td>
